...
export const useAuthStore = defineStore('auth', {
  ...
  getters: {
    ...
    isS2: (state) => ['s2', 'transporte'].includes(state.role),
    ...
    canHomologar: (state) => ['s2', 'transporte'].includes(state.role),
    // Pode excluir militares permanentemente
    canDelete: (state) => state.role === 'admin',
    // Pode ver relatório S2
    canViewS2Report: (state) => ['admin', 's2', 'transporte'].includes(state.role),
  },
  actions: {
